...
import {
  ...
  PlaceType,
  UserData,
  FavoriteCardStatusType,
} from '../types/types';
import { ApiRoute, AppRoute, AUTH_INFO, AuthorizationStatus } from '../const';
import {
  loadOffersAction,
...
>('data/loadOffersAction', async (_arg, { dispatch, extra: api }) => {
...
>('data/loadOfferDetailsAction', async (offerId, { dispatch, extra: api }) => {
  dispatch(setDataLoadingStatusAction(true));
...
>('data/loadFavoriteCardsAction', async (_arg, { dispatch, extra: api }) => {
...
>('data/createNewFavorite', async (payload, { extra: api }) => {
...
>('data/loadCommentsAction', async (offerId, { dispatch, extra: api }) => {
  dispatch(setDataLoadingStatusAction(true));
...
>('data/loadNearByOffersAction', async (offerId, { dispatch, extra: api }) => {
  dispatch(setDataLoadingStatusAction(true));
  const { data } = await api.get<PlaceType[]>(
...
>('user/requareAuthAction', async (_arg, { dispatch, extra: api }) => {
  try {
...
>('user/logoutAction', async (_arg, { dispatch, extra: api }) => {
